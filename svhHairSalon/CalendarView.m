...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        //NSLog(@"NSDate == %@", _calendarDate);      //제대로 날짜 나옴...
...
-(void)setCalendarDate:(NSDate *)calendarDate
{
    ...
    //NSLog(@"calendarDate == %@", calendarDate);     //날짜 제대로 나옴...
    [self setNeedsDisplay];
}
...
-(IBAction)tappedDate:(UIButton *)sender
{
    ...
    //NSLog(@"_calendarDate == %@", _calendarDate);
    ...
    if (sender.tag < 0 || sender.tag >= 40)
    {
        ...
        if ([self canSwipeToDate:otherMonthDate])
        {
            ...
            ////////
            //NSDateComponents * components = [_gregorian components:_dayInfoUnits fromDate:[_selectedDate initWithTimeInterval:24*60*60 sinceDate:_selectedDate]];
    ...
    if(componentsDateSel.day != sender.tag || componentsDateSel.month != components.month || componentsDateSel.year != components.year)
    {
        ...
        //[_delegate dayChangedToDate:[_selectedDate initWithTimeInterval:24*60*60 sinceDate:_selectedDate]]; //다음날 더하기
        [_delegate dayChangedToDate:_selectedDate]; //다음날 더하기
    }
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSInteger weekdayBeginning      = [comps weekday]+1;  // Starts at 1 on Sunday
    weekdayBeginning -=2;
